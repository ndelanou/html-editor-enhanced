...
import '../../html_editor.dart'
    hide ContextMenu, NavigationActionPolicy, UserScript;
import '../../utils/utils.dart';

/// The HTML Editor widget itself, for mobile (uses InAppWebView)
class HtmlEditorWidget extends StatefulWidget {
  const HtmlEditorWidget({
    required this.controller, required this.plugins, required this.htmlEditorOptions, required this.htmlToolbarOptions, required this.otherOptions, super.key,
    this.callbacks,
  });
...
class _HtmlEditorWidgetMobileState extends State<HtmlEditorWidget> {
                ...
                child: InAppWebView(
                  ...
                  onWebViewCreated: (InAppWebViewController controller) {
                    ...
                    controller.addJavaScriptHandler(
                        ...
                        callback: (e) {
                          final json = e[0] as Map<String, dynamic>;
                  ...
                  onWindowFocus: (controller) async {
                    ...
                    if (widget.htmlEditorOptions.adjustHeightForKeyboard &&
                      ...
                      final visibleDecimal = await visibleStream.stream.first;
                      final newHeight = widget.otherOptions.height;
                  ...
                  onLoadStop:
                    ...
                    if (url.contains(filePath)) {
                      ...
                      final editorController = widget.controller.editorController as InAppWebViewController;
                      editorController.addJavaScriptHandler(
                          handlerName: 'totalChars',
                          callback: (keyCode) {
                            final key = keyCode.first as int;
                            widget.controller.characterCount = key;
                          },);
  ...
  void addJSCallbacks(Callbacks c) {
    if (c.onBeforeCommand != null) {
      widget.controller.editorController!.evaluateJavascript(source: r"""
  ...
  void addJSHandlers(Callbacks c) {
    ...
    if (c.onImageUpload != null) {
      editorController.addJavaScriptHandler(
          ...
          callback: (files) {
            final jsonFile = files.first as String;
            final file = fileUploadFromJson(jsonFile);
    ...
    if (c.onImageUploadError != null) {
      editorController.addJavaScriptHandler(
          ...
          callback: (args) {
            if (args is! List<String>) throw Exception('args is not a List<String>');
            ...
            if (!args.first.startsWith('{')) {
              ...
              final file = fileUploadFromJson(args.first);
              c.onImageUploadError!.call(
                  file,
                  null,
                  args.last.contains('base64')
