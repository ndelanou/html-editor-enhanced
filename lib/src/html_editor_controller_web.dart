...
class HtmlEditorController extends unsupported.HtmlEditorController {
  ...
  Future<String> getText() async {
    await _evaluateJavascriptWeb(data: {'type': 'toIframe: getText'});
    final e = await html.window.onMessage.firstWhere((element) =>
        (json.decode(element.data as String) as Map<String, dynamic>)['type'] == 'toDart: getText',);
    var text = (json.decode(e.data as String) as Map<String, dynamic>)['text'] as String;
  ...
  Future<String> getSelectedTextWeb({bool withHtmlTags = false}) async {
    if (withHtmlTags) {
      await _evaluateJavascriptWeb(data: {'type': 'toIframe: getSelectedTextHtml'});
    } else {
      await _evaluateJavascriptWeb(data: {'type': 'toIframe: getSelectedText'});
    }
    final e = await html.window.onMessage.firstWhere((element) =>
        (json.decode(element.data as String) as Map<String, dynamic>)['type'] ==
        'toDart: getSelectedText',);
    return (json.decode(e.data as String) as Map<String, dynamic>)['text'] as String;
  }
  ...
  void setText(String text) {
    final processedText = _processHtml(html: text);
    _evaluateJavascriptWeb(data: {'type': 'toIframe: setText', 'text': processedText});
  }
  ...
  void insertHtml(String html) {
    ...
    _evaluateJavascriptWeb(
        data: {'type': 'toIframe: insertHtml', 'html': processedHtml},);
  }
  ...
  Future<dynamic> evaluateJavascriptWeb(String name,
    ...
    await _evaluateJavascriptWeb(data: {'type': 'toIframe: $name'});
    if (hasReturnValue) {
      final e = await html.window.onMessage.firstWhere((element) =>
          json.decode(element.data as String)['type'] == 'toDart: $name',);
      return json.decode(e.data as String);
    }
  }
  ...
  Future<void> _evaluateJavascriptWeb({required Map<String, Object?> data}) async {
    if (kIsWeb) {
      data['view'] = _viewId;
